...
class XEMC
{
	public:
		...
		int Process(const double kE0, const double kEp, const double kTheta, const int kA, const int kZ)
		{ 
			if(kE0>25.0||kEp>25.0){
				cerr<<" *** Are you sure? E0 or Ep > 25GeV?!!! *** "<<endl;
				return -1;
			}
			...
			err=Event->Run();
			if ( err==-1 ){
				cerr << "********** Fail to calculate XS!!!"<<endl;
				xs_rad = -1111.00;
				xs_dis = -1112.00;
				xs_qe = -1113.00;
				xs_born = -2222.00;
				...
				xs_el = Event->cs_el; // added bt Shujie, 2018.04
		...
		void Init (const string kTarget_Input){
			...
			//	cout<<kTarget_Input<<"2222222222"<<endl;
			char buf[CHAR_LEN];
			char data[CHAR_LEN];
	...
	private:
		...
		int err;
		XEMCEvent* Event;
		double xs_rad, xs_born, xs_mott,xs_qe,xs_dis,xs_el;
