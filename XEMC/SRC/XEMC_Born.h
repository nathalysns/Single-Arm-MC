...
bool IsGoodFlag(int kflag){
  ...
  if(kflag==1||kflag==2||kflag==3||kflag==4||kflag==5||kflag==6||kflag==7) 
    return kTRUE;
  else
    return kFALSE;  
...
Double_t XEMC_Born(double iE0set, double iEpset, double iTheta, int iA,int iZ,int iFlag)
{
 ...
 // Bool_t bCCor = kFALSE; //Give an option to calculate Coulomb Correction
  Bool_t bCCor = kTRUE; // turn on the CC 
...
//      iTarget->Print();
  ...
  if(IsGoodFlag(iFlag)) 
     ...
     cerr<< "iFlag =1->QE(Fy)+DIS(F1F2IN09), iFlag=2->QE(Fy) Only, iFlag=3->DIS Only (F1F2IN09), iFlag==4->DIS Only (F1F2IN06)"<<endl;
	 cerr<< "iFlag =5->QE(Fy)+DIS(F1F2IN09)+Coulomb_Correction, iFlag=6->QE(Fy)+DIS(F1F2IN06), iFlag=7->QE(F1F2QE09)+DIS(F1F2IN09)"<<endl;
