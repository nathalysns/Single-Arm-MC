inline void gCal_Fy2Sig( double aE0,double aEp, double aTheta, XEM_TGT* aTarget, XEM_SIG* fy2sig)
{
  ...
  */
  ...
  if(y_cal<-99)//If there is no solution for y, else y=-100000
	  sig_out = 0.0;
  else  if(x_local > aTarget->A )//Remove x > A case since QE won't allow to go there, Z. Ye 01/24/2014
	  sig_out = 0.0;
  else{
	  sig_out = fact*gGet_Fy(y_cal,aTarget)*1e6;

	  if(aTarget->A==3 &&aTarget->Z==2&&x_local>=2.80){ // New twist for He3 to prevent it drops to zero when closing x=3, -- Z. Ye 02/05/2015
	    double zhihong_hack = pow(10., (2.4-1.936*x_local)) / pow(10., (-32.96*x_local*x_local+182.43*x_local-255.50));
		sig_out *= zhihong_hack;
	  }
  	  if(aTarget->A==4 &&aTarget->Z==2&&x_local>=3.40){ // New twist for He4 to prevent it drops to zero when closing x=4, -- Z. Ye 02/06/2015
	    double zhihong_hack = pow(10., (3.537-2.191*x_local)) / pow(10., (-12.31*x_local*x_local+81.95*x_local-140.25));
		sig_out *= zhihong_hack;
	  }
}
...
inline Double_t gGet_Y(double vE0, double vEp, double vTheta, XEM_TGT* vTarget)
{
  ...
  double y_plus = -(coeff_b+sqrt(root2))/(2.0*coeff_a);
  // double y_minus= -(coeff_b-sqrt(root2))/(2.0*coeff_a);
...
inline Double_t gGet_He3_Fy( double vY, XEM_TGT* vTarget, const double vY_Min_He3){
  ...
  if(vY>vY_Min_He3)
	  ...
	  y=1000.0*vY_Min_He3; //Add a flat line after y<=-0.8 to prevend the Fy falls into zero quickly
