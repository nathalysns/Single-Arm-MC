...
inline void gCal_Sigma(double kE0,double kEp,double kTheta,XEM_TGT *kTarget,bool kCCor, int kFlag, XEM_XS* sig)
{
  ...
  //      E_SEP:  - Separation energy for target nucleus in GeV/c1.
...
// also, all deltae_cc's are computed for Z-1, not Z!/-*    
  ...
  //compute the uncertainty from V_eff = 0.75~0.8V. --LSJ
 // DeltaE = DeltaE/0.775*0.75;//lower limit
//  DeltaE = DeltaE/0.775*0.8;//upper limit
// DeltaE = 0;
//cout<< DeltaE<<"###############################"<<endl;	
...
//cout<< "dE = "<< DeltaE<<"  sig = "<< sig->QE+sig_dis<<endl;
