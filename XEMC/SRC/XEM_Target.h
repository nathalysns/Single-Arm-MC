...
//char* XEMC_DIR = std::getenv("XEMDIR");
TString XEMC_DIR = "./SRC";
const TString TARGET_TABLE = Form("%s/target.table",XEMC_DIR.Data());
...
class XEM_TGT
{
 ...
 //   GetValueAZ();
  ...
  void GetAZ(const TString& kName, Int_t& A, Int_t& Z){
     Exist = kFALSE;
    ifstream table;
  ...
  void GetValue( const TString& kName ){
    ...
    if(Exist)
      ...
      cerr << "****** ERROR, your target is not in the table! Check the following table:" <<kName<<endl;
      //  List();
    }
  ...
  void GetValueAZ( const int kA, const int kZ ){
    ...
    if(!(Exist)){
      //     cerr <<endl<< "@@@@@@ I have found your target from the table!" <<endl;
      //  else{
      cerr << "****** ERROR, your target is not in the table! Check the following table:" <<kA<<" "<<kZ<<endl;
      // List();
    }
    //Be careful -- the following variables needed to be converted:
    Mass *= U2GeV ; //From u to GeV;
